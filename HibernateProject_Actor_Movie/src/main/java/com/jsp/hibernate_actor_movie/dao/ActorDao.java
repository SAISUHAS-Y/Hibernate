...
public class ActorDao {
	...
	public void updateAllActorSalaryByMovieTitle() {
		...
		query.where(builder.equal(root.get("movieTitle"),movieTitle ));
			...
			for (Movie movie : movieList) {
				...
				if(actor!=null) {
					for(Actor a:actor) {
						...
						query1.where(builder.equal(root.get("movieTitle"), movieTitle));
						
						Query updateQuery=em.createQuery(query1);
						int rowUpdated=updateQuery.executeUpdate();
						if(rowUpdated>0) {
							System.out.println(rowUpdated+"data get Updated");
						}
						else {
							System.err.println("Data is not updated for some reason");
						}
						
					}
				}
				else {
					System.err.println("Actor not found to update");
				}
	...
	public void deleteAllActorByMovieGenre() {
		...
		if (movieList.isEmpty()) {
			...
			for (Movie movie : movieList) {
				...
				if (actor.isEmpty()) {
					for (Actor a : actor) {
						Query query1 = em.createQuery("delete from Actor a where a.actorId=:id");
						query1.setParameter("id", a.getActorId());
						int resDelete = query1.executeUpdate();
						if (resDelete > 0) {
							System.out.println(resDelete + " data is deleted");
						} else {
							System.err.println("Data is not deleted");
						}
					}
				} else {
					System.err.println("actor is not found with genre");
				}
