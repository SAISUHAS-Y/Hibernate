...
public class MovieDao {
	...
	public void updateMovieVerdictBetweenCollection() {
		emf = Persistence.createEntityManagerFactory("mysql-config");
		EntityManager em = emf.createEntityManager();
		EntityTransaction et = em.getTransaction();
		et.begin();
		...
		cq.where(builder.between(root.get("movieCollection"), sc.nextInt(), sc.nextInt()));
		
		Query query=em.createQuery(cq);
		List<Movie> movie=query.getResultList();
		System.out.println("Enter the verdit");
		String verdit=sc.next();
		for(Movie m:movie) {
			m.setMovieVerdict(verdit);
		}
	...
	public void deleteMoviesByVerdict() {
		emf = Persistence.createEntityManagerFactory("mysql-config");
		EntityManager em = emf.createEntityManager();
		EntityTransaction et = em.getTransaction();
		et.begin();
		System.out.println("Enter the verdict");
		Query query=em.createQuery("delete from Movie a where a.movieVerdict=:a");
		query.setParameter("a", sc.next());
		
		int rowDelete=query.executeUpdate();
		if(rowDelete>0) {
			System.out.println(rowDelete+" data get deleted");
		}
		else
		{
	...
	public void deleteMoviesByActorId() {
		emf = Persistence.createEntityManagerFactory("mysql-config");
		EntityManager em = emf.createEntityManager();
		EntityTransaction et = em.getTransaction();
		et.begin();
		...
		cq.where(builder.equal(root.get("actorId"), sc.nextInt()));
		
		Query query=em.createQuery(cq);
		Actor actors=(Actor) query.getSingleResult();
	    List<Movie> movies=	actors.getMovies();
